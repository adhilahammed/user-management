...
import { loginUserAction } from "../../redux/slice/userSlices/userSlices";  
...
const Login = () => {
  ...
  const store=useSelector(state=>state?.admin)  
   const {loading,appErr,serverErr,userAuth,userLogin}=store
  console.log(appErr);

  
  //redirect
  if (userLogin) return <Navigate to="/list" />;
                ...
                <div className="px-6 lg:px-12 py-12 lg:py-24 bg-white shadow-lg rounded-lg">
                  ...
                  <form onSubmit={formik.handleSubmit}>
                    <h3 style={{'textAlign':'center'}} className="mb-10 text-2xl font-bold font-heading">
                      {/* Header */}
                      Login to your Account ok
                      {appErr || serverErr? <div className="text-red-800">
                  {serverErr} {appErr} -invalid login credentials
                  </div>:null}
                    </h3>
                 ...
                 <span>start now if you are new?</span>
                 <a href="/register" style={{'color':'blue','text-decoration': 'underline'}}> sign up</a>
                 </div>
                 {/* } */}
                  </form>
                </div>
